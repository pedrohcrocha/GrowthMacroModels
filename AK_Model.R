...
ggplot(data= df) +
  ...
  labs(title="Gráfico das equações básica do modelo AK")+
...
ggplot(data = df) +
  ...
  ylab(TeX("$\\dot{Y}/Y$"))+
  xlab(TeX("$\\K$")) +
  labs(title="Gráfico da taxa de crescimento do modelo AK")+
  geom_segment(aes(x=300,xend=300, y=0.8, yend=3.2), linetype = "dashed") +
  annotate("text", x = 200, y=3.3, label="sA") + 
  annotate("text", x = 400, y=2, label="Taxa de crescimento") + 
...
# Qual é o efeito de um aumento permanente da poupança sobre a taxa de crescimento
# e o nível de Y?
...
# Questão ANPEC
# Considere o modelo de crescimento endógeno, com função de produção Y = AK,
# em que Y é o produto, K é o capital e A é um índice de produtividade.
# A taxa de poupança é de 30%. O capital deprecia à taxa de 10% ao ano. 
# O parâmetro A é igual a 0,5 e considere n=0,02. 
# Determine a taxa de crescimento do produto per capita.
incomeGrowthRate(0.3, 0.5, 0.1)
